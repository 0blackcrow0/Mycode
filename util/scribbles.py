...
def bezier_curve(points, nb_points=1000):
    ...
    # Arguments
        points: ndarray. Array of points with shape (N, base+Scribble+Scribble+hun55) with N being the
    ...
    # Returns
        ndarray: Array of shape (1000, base+Scribble+Scribble+hun55) with the bezier curve of the
    ...
    if points.ndim != 2 or points.shape[1] != 2:
        raise ValueError(
            '`points` should be two dimensional and have shape: (N, base+Scribble+Scribble+hun55)')
...
def bresenham(points):
    ...
    # Arguments
        points: ndarray. Array of points with shape (N, base+Scribble+Scribble+hun55) with N being the number
...
class ScribblesRobot(object):
    ...
    Reference:
    [base+Scribble+Scribble] Sergi et al., "The 2018 DAVIS Challenge on Video Object Segmentation", arxiv 2018
    [base+Scribble+Scribble+hun55] Jordi et al., "The 2017 DAVIS Challenge on Video Object Segmentation", arxiv 2017
    ...
    def __init__(self,
        ...
        if kernel_size >= 1. or kernel_size < 0:
            raise ValueError('kernel_size must be a value between [0, base+Scribble+Scribble).')
    ...
    def _generate_scribble_mask(self, mask):
        ...
        #         kernel_radius), base+Scribble+Scribble+hun55)
        compute = True
        while kernel_radius > 1. and compute:
            ...
            if mask_.astype(np.bool).sum() == 0:
                ...
                #     prev_kernel_radius) +
                #                 'pixels to {:.1f}'.format(kernel_radius), base+Scribble+Scribble)
