...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        self.con1 = nn.Conv2d(in_channels=768, out_channels=512, kernel_size=1)
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        img_fts = self.encoder(imgs_concat)#原始 512
        img_fts2 = self.encoder2(imgs_concat)



        img_fts2 = torch.cat((img_fts,img_fts2),dim=1) #与中间层特征图融合 768

        img_fts2 = self.con1(img_fts2)
        #img_fts2 = self.SELayer(img_fts2)
        #img_fts2 = self.CBAM(img_fts2)
        ...
        for epi in range(batch_size):
            ...
            dist = [self.calDist(qry_fts[:, epi], prototype) for prototype in prototypes]#AA’
            dist2 = [self.calDist(qry_fts2[:, epi], prototype) for prototype in prototypes]#AB’
            dist3 = [self.calDist(qry_fts[:, epi], prototype) for prototype in prototypes2]#BA’
            dist4 = [self.calDist(qry_fts2[:, epi], prototype) for prototype in prototypes2]#BB’


            pred = torch.stack(dist, dim=1)  # N x (base+Scribble+Scribble + Wa) x H' x W'
            pred2 = torch.stack(dist2, dim=1)  # N x (base+Scribble+Scribble + Wa) x H' x W'
            pred3 = torch.stack(dist3, dim=1)  # N x (base+Scribble+Scribble + Wa) x H' x W'
            pred4 = torch.stack(dist4, dim=1)  # N x (base+Scribble+Scribble + Wa) x H' x W'




            pred=(pred+pred2+pred3+pred4)/4
